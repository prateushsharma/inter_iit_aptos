...
function App() {
  ...
  const playlistexist = async ()=> {
    await fetchPlaylists();
    if(playlists.length > 0) {
      return true;
    }
    else{
      message.error("No playlist exist kindly create one");
      setTimeout(() => {
        message.error("dfsyguhdfuhifiufsi");
        // <Link className="nav-link btn btn-light" to="/playlistpage">Earnings</Link>
